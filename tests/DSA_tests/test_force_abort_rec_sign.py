...
def test_signature_validity_fail():
    ...
    with mock.patch.object(PoC_DSA.src.entities.RecoveryParty, 'combine', new=mock_combine_rec):
        try: 
...
def test_combine_commitment_verification_fail():
    def mock_combine_rec(self, other_s_decommitment):
        ...
        # Verify the commitment received from the other user
        if not PoC_DSA.src.crypto_utils.verify_commitment(self.other_s_commitment, self.other_s_decommitment):
            # The protocol aborts
            PoC_DSA.src.general_procedures.abort()
        else:
            self.s = (self.s_3 + self.other_s_decommitment[0]) % self.q
...
def test_signature_3_part_2_R_equals_1_fail():
    ...
    with mock.patch.object(PoC_DSA.src.entities.RecoveryParty, 'signature_3_part2', new=mock_signature_3_part2_rec):
        try: 
            PoC_DSA.src.main.main()
        except PoC_DSA.src.utils.ProtocolAbortedException:
            assert True
...
def test_signature_validity_fail_other_R_equals_1():
    ...
    with mock.patch.object(PoC_DSA.src.entities.RecoveryParty, 'signature_3', new=mock_signature_3_rec):
        try: 
            PoC_DSA.src.main.main()
        except PoC_DSA.src.utils.ProtocolAbortedException:
            assert True
