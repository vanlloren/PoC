...
def test_hash_message_truncation():
    ...
    while True:
        ...
        combined = context_info + counter + len(nonce_bytes).to_bytes(8, byteorder='little') + nonce_bytes + len(message_bytes).to_bytes(8, byteorder='little') + message_bytes + (32).to_bytes(8, byteorder='little')  # output_length is 32 bytes (256 bits)

        hash_digest = hashlib.shake_256(combined).digest(32)
        counter = counter[0] + 1  # Increment the counter for the next hash if needed
        if hash_digest != b'\x00' * 32:  # Ensure we don't get an all-zero hash, which would be invalid
            break
