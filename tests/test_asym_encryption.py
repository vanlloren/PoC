...
def test_rsa_encryption_decryption():
    ...
    private_key, public_key = src.crypto_utils.generate_rsa_keypair()

    # Define a plaintext integer (e.g., a hash value)
    plaintext = 12345678901234567890123456789012345678

    # Encrypt the plaintext using the public key
    ciphertext = src.crypto_utils.encrypt_with_public_key(public_key, plaintext)

    # Decrypt the ciphertext using the private key
    decrypted = src.crypto_utils.decrypt_with_private_key(private_key, ciphertext)
...
def test_max_256bit_message():
        ...
        ciphertext = src.crypto_utils.encrypt_with_public_key(public_key, max_256bit)
        decrypted = src.crypto_utils.decrypt_with_private_key(private_key, ciphertext)
...
def test_min_256bit_message():
        ...
        ciphertext = src.crypto_utils.encrypt_with_public_key(public_key, min_message)
        decrypted = src.crypto_utils.decrypt_with_private_key(private_key, ciphertext)
...
def test_encrypt_large_message():
    ...
    with pytest.raises(ValueError) as exc_info:
        src.crypto_utils.encrypt_with_public_key(public_key, large_message)
...
def test_encrypt_negative_message():
    ...
    with pytest.raises(ValueError) as exc_info:
        src.crypto_utils.encrypt_with_public_key(public_key, negative_message)
...
def test_encrypt_non_integer():
    ...
    with pytest.raises(ValueError) as exc_info:
        src.crypto_utils.encrypt_with_public_key(public_key, non_integer_message)
...
def test_decrypt_with_wrong_key():
    ...
    private_key1, public_key1 = src.crypto_utils.generate_rsa_keypair()
    private_key2, public_key2 = src.crypto_utils.generate_rsa_keypair()
    
    plaintext = 12345678901234567890123456789012345678
    
    # Encrypt with the first public key
    ciphertext = src.crypto_utils.encrypt_with_public_key(public_key1, plaintext)
    
    # Decrypt with the second private key (should not match original plaintext)
    with pytest.raises(ValueError) as exc_info:
        src.crypto_utils.decrypt_with_private_key(private_key2, ciphertext)
...
def test_encryption_non_deterministic():
    _, public_key = src.crypto_utils.generate_rsa_keypair()
    message = 42424242
    
    cipher1 = src.crypto_utils.encrypt_with_public_key(public_key, message)
    cipher2 = src.crypto_utils.encrypt_with_public_key(public_key, message)
...
def test_ciphertext_uniformity():
    ...
    ciphertexts = [src.crypto_utils.encrypt_with_public_key(public_key, message) for _ in range(100)]
